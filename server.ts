  ...
  try {
    ...
    if (local.handled && local.reply) {
      ...
      res.json({
        ...
        speechIdentity: speech,
        kdm: { trace: kdm.trace, dynamicState: kdm.nextDynamicState, semanticEvent: canonicalSemantic.event, semanticSource: canonicalSemantic.source, entityResolution: languageUnderstanding.entityResolution, worldEvent: languageUnderstanding.worldEvent, retrievedWorldEvents: retrievedWorldEvents.map((item) => ({ id: item.observation.id, score: item.score, kind: item.observation.kind, status: item.observation.status, event: item.observation.event })), worldStateAppraisal, worldReasoningPolicy, worldMemoryGuard, behaviorContract, behaviorProfile, responsePlan },
        consistency,
    ...
    if (postEnforcementPlanIssues.length) {
      ...
      if (planSafeFallback) {
        const planSafeIssues = [
          ...
          ...findKairoResponseRhythmIssues(planSafeFallback, cleanHistory, dialogueDecision.move),
