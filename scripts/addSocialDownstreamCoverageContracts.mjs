...
const testPath = 'src/services/kairaSocialDownstreamCoverageContracts.test.ts';
...
describe('social orientation downstream coverage', () => {
  ...
  it('does not let high social openness erase betrayal-driven distance', () => {
    const response = computeSocialOrientationResponse({ warmth: 100, empathy: 100, closenessDrive: 100, dominance: 50, initiative: 100, compliance: 100, initialTrust: 100, disclosure: 100 }, sit({ betrayalSignal: 1 }), {
    ...
    expect(integrate(response).decision.distance).toBeGreaterThan(0.2);
  });
});
`);
if (!fs.readFileSync(testPath, 'utf8').includes('social orientation downstream coverage')) throw new Error('missing social coverage marker');
console.log('Added social downstream coverage contracts');
