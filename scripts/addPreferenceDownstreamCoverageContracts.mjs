...
const testPath = 'src/services/kairaPreferenceDownstreamCoverageContracts.test.ts';
...
`);
if (!fs.readFileSync(testPath, 'utf8').includes('preference downstream coverage')) throw new Error('missing preference coverage marker');
console.log('Added preference downstream coverage contracts');
