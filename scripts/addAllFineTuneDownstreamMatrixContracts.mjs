...
const testPath = 'src/services/kairaAllFineTuneDownstreamMatrixContracts.test.ts';
...
if (!fs.readFileSync(testPath, 'utf8').includes('all fine-tune downstream matrix')) {
  throw new Error('missing all-fine-tune matrix marker');
}
console.log('Added all fine-tune downstream matrix contracts');
