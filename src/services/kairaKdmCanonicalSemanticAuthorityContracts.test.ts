...
describe("KDM canonical SemanticInterpretation@2 authority", () => {
  it("receives the ingestion-time interpretation, deterministic event projection, behavior policy, affect baseline, and bounded appraisal memory", () => {
    const server = source("server.ts");
    expect(server).toContain('import { analyzeKdmInteractionCanonicalTurn } from "./src/services/kdmConsistencyEngine"');
    expect(server).toMatch(/analyzeKdmInteractionCanonicalTurn\(\s*userMessage,\s*basePersonality,\s*effective,\s*canonicalSemantic\.interpretation,\s*canonicalSemantic\.event,\s*behaviorPolicy,\s*affectBaseline,\s*socialAppraisalMemoryRuntime\.memory,\s*\)/u);
