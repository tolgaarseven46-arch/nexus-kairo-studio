...
import { decideResponseRepair, selectBestConsistency } from './kdmResponseRepairPolicy';
...
describe('KDM response consistency gate', () => {
  it('accepts a response matching the emotional context and tone', () => {
    const result = validateKairoResponse('Anlıyorum, buradayım. Birlikte bunu çözebiliriz.', trace());
  ...
  it('rejects an emotionally mismatched response', () => {
    const result = validateKairoResponse('Net konuşuyorum: Kural bu, uygulanmalı.', trace());
  ...
  it('never replaces a better response with a worse repair', () => {
    const original = validateKairoResponse('Anlıyorum, buradayım ve destek olmak istiyorum.', trace());
