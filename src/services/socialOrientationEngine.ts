...
export interface SocialOrientationResponse {
  effective: { warmth: number; empathy: number; closeness: number; dominance: number; initiative: number; compliance: number; trustOpenness: number; disclosure: number };
  behaviorSignals: { affiliationPressure: number; carePressure: number; leadershipPressure: number; resistancePressure: number; disclosurePressure: number; socialDistancePressure: number };
...
export const DEFAULT_SOCIAL_ORIENTATION_PROFILE: SocialOrientationProfile = {
  warmth: 50, empathy: 50, closenessDrive: 50, dominance: 50, initiative: 50, compliance: 50, initialTrust: 50, disclosure: 50,
};

export const socialOrientationFromFineTune = (fineTune: Record<string, number> | null | undefined): SocialOrientationProfile => {
  const p = fineTune ?? {};
  const read = (key: string) => clamp100(p[key] ?? 50);
  return {
    warmth: read("social.communion.warmth"), empathy: read("social.communion.empathy"), closenessDrive: read("social.communion.closenessDrive"),
    dominance: read("social.agency.dominance"), initiative: read("social.agency.initiative"), compliance: read("social.agency.compliance"),
    initialTrust: read("social.trust.initialTrust"), disclosure: read("social.trust.disclosure"),
  };
};
...
export const computeSocialOrientationResponse = (
  ...
  const safety = clamp01(trustHistory * 0.5 + warmthHistory * 0.3 + familiarity * 0.2 - hurt * 0.35 - conflict * 0.25 - betrayal * 0.4);
  const warmth = clamp100(profile.warmth * (0.72 + affiliation * 0.2 + vulnerability * 0.12 + safety * 0.12 - conflict * 0.2));
  const empathy = clamp100(profile.empathy * (0.75 + vulnerability * 0.3 + affiliation * 0.08 - coercion * 0.08));
  const closeness = clamp100(profile.closenessDrive * (0.62 + affiliation * 0.2 + intimacy * 0.2 + safety * 0.22 - hurt * 0.25 - betrayal * 0.35));
  const dominance = clamp100(profile.dominance * (0.74 + challenge * 0.18 + coercion * 0.24 + request * 0.06));
  const initiative = clamp100(profile.initiative * (0.72 + affiliation * 0.12 + vulnerability * 0.12 + request * 0.16));
  const compliance = clamp100(profile.compliance * (0.76 + request * 0.16 - coercion * (0.25 + n(profile.dominance) * 0.2) - challenge * 0.08));
  const trustOpenness = clamp100(profile.initialTrust * (0.5 + safety * 0.55 - betrayal * 0.45 - hurt * 0.2));
  const disclosure = clamp100(profile.disclosure * (0.5 + intimacy * 0.22 + safety * 0.32 + familiarity * 0.12 - conflict * 0.25 - betrayal * 0.35));

  const affiliationPressure = clamp01(n(warmth) * 0.35 + n(closeness) * 0.35 + affiliation * 0.3);
  const carePressure = clamp01(n(empathy) * 0.5 + vulnerability * 0.5);
  ...
  return {
    effective: { warmth, empathy, closeness, dominance, initiative, compliance, trustOpenness, disclosure },
    behaviorSignals: { affiliationPressure, carePressure, leadershipPressure, resistancePressure, disclosurePressure, socialDistancePressure },
    legacyTraits: {
      empathy, authority: dominance, initiative,
