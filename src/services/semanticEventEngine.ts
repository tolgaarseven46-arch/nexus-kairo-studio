...
// Bu yüzden kelime sınırlarını Unicode harf/rakam negatif lookaround ile kuruyoruz.
const word = (source: string) => new RegExp(`(?<![\\p{L}\\p{N}_])(?:${source})(?![\\p{L}\\p{N}_])`, "u");
...
const INSULT_RE = new RegExp(`${word("aptal|salak|gerizekalı|mal|şerefsiz|haysiyetsiz|ezik|piç|yavşak|köle").source}|geri zekalı|siktir|defol|boş konuş`, "u");
...
const REJECTION_RE = /(istemiyorum|git başımdan|bırak beni|kaybol|defol)/u;
...
const FRUSTRATION_RE = /(yeter|bıktım|sinir|aynı şeyi|kaç kere|neden anlamıyorsun|niye anlamıyorsun|hala soruyorsun|hâlâ soruyorsun|soru sorma)/u;
const VENTING_PROFANITY_RE = word("amk|aq|mk");
const EMOTIONAL_SHARE_RE = /(moralim.{0,30}bozuk|üzgünüm|çok mutluyum|mutluyum|bunaldım|canım (?:çok )?sıkkın|kendimi (?:çok )?kötü hissediyorum|kendimi (?:çok )?iyi hissediyorum|kaygılıyım|endişeliyim|yoruldum|tükendim)/u;
const LOW_MOOD_RE = /(moralim.{0,30}bozuk|üzgün|kötü hissed|bunaldım|canım (?:çok )?sıkkın|kaygı|endişe|stres|yoruldum|tükendim)/u;
const INFORMATION_REQUEST_RE = /(?:^|\s)(neden|niye|nasıl|nedir|ne demek|kim|kime|kimi|hangi|hangisi|nerede|neresi|neydi|ne yapacaktı)(?:\s|$|[?.!,])/u;

function inferTarget(text: string, negative: boolean, insult: boolean, rejection: boolean): SemanticTarget {
  ...
  if (RED_LINE_RE.test(text) || insult || rejection || COERCION_RE.test(text) || MANIPULATION_RE.test(text)) return "kaira";
  return "event";
}
...
export function interpretSemanticEvent(message: string): SemanticEvent {
  ...
  const frustration = FRUSTRATION_RE.test(text) ? 0.75 : VENTING_PROFANITY_RE.test(text) ? 0.35 : 0;
  ...
  else if (stopQuestions || stopTalking || frustration >= 0.7) intent = "complaint";
