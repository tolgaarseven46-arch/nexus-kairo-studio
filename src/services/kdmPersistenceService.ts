...
import type { SemanticInterpretation, SemanticProposition } from '../types/semanticInterpretation';
...
export function sanitizeKntTracePayloadForPersistence(
  payload: KntTracePayload,
  userScope: string,
): KntTracePayload & { userId: string; createdAt: string } {
...
export async function saveKntTrace(payload: KntTracePayload): Promise<void> {
  const userScope = scope(payload.userId);
  const stateRef = doc(db, STATE_COLLECTION, userScope);
  await addDoc(
    collection(stateRef, KNT_COLLECTION),
    sanitizeKntTracePayloadForPersistence(payload, userScope),
  );
}
