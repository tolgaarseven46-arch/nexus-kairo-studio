...
type RepairOracle = {
  type: "repair_requires_source";
  watchFromTurn: number;
};
type ThirdPartyOracle = {
  type: "third_party_neutrality";
  thirdPartyTurns: number[];
};
type PairedRelationshipOracle = {
  type: "paired_relationship_difference";
};
type NaturalOracle =
  | StopResumeOracle
  | RepairOracle
  | ThirdPartyOracle
  | PairedRelationshipOracle;
...
function scenarioForHarness(
  ...
  return {
    ...
    cluster: "D",
    // v2 is not part of the frozen regression-readiness baseline.
...
function stateProjection(state: DroitDynamicState) {
  return {
    calmness: Number(state.calmness ?? 0),
    ...
    surprise: Number(state.surprise ?? 0),
...
function relationshipProjection(relationship?: RelationshipState) {
  ...
  return {
    familiarityDays: Number(relationship.familiarityDays ?? 0),
    interactionCount: Number(relationship.interactionCount ?? 0),
    warmthScore: Number(relationship.warmthScore ?? relationship.warmth ?? 50),
    trustScore: Number(relationship.trustScore ?? relationship.trust ?? 50),
    conflictScore: Number(relationship.conflictScore ?? 0),
    hurtScore: Number(relationship.hurtScore ?? 0),
    repairProgress: Number(relationship.repairProgress ?? 0),
    toleranceMultiplier: Number(relationship.toleranceMultiplier ?? 1),
    repeatedNegativeCount: Number(relationship.repeatedNegativeCount ?? 0),
...
function compactTurn(
  scenario: NaturalCharacterizationScenarioDefinition,
  variantId: string,
  turn: KairaPreAiScenarioTurnResult,
): NaturalCharacterizationTurnArtifact {
  const interp = turn.interpretation;
  const plan = turn.responsePlan;
  return {
    scenarioId: scenario.scenarioId,
    variantId,
    openQuestionId: scenario.openQuestionId,
    turnNumber: turn.turnNumber,
    userMessage: turn.userMessage,
    semanticSource: turn.semanticSource,
    semantic: {
      primaryIntent: String(interp.primaryIntent),
      target: String(interp.target),
      valence: String(interp.valence),
      jokingConfidence: Number(interp.jokingConfidence ?? 0),
      sincerityConfidence: Number(interp.sincerityConfidence ?? 0),
      emotionalLoad: Number(interp.emotionalLoad ?? 0),
      repairAttempt: interp.repairAttempt === true,
      apology: interp.apology === true,
      severity: { ...interp.severity },
    },
    dynamicState: {
      before: stateProjection(turn.dynamicStateBefore),
      after: stateProjection(turn.dynamicStateAfter),
    },
    relationship: {
      before: relationshipProjection(turn.dynamicStateBefore.relationship),
      after: relationshipProjection(turn.dynamicStateAfter.relationship),
    },
    dialogue: {
      move: turn.dialogueDecision?.move ?? null,
      target: turn.dialogueDecision?.target ?? null,
      obligation: turn.dialogueDecision?.obligation?.type ?? null,
    },
    responsePlan: {
      resolver: plan?.resolver ?? null,
      continueConversation: plan?.continueConversation === true,
      allowQuestion: plan?.allowQuestion === true,
      allowHumor: plan?.allowHumor === true,
      allowAffection: plan?.allowAffection === true,
      allowAdvice: plan?.allowAdvice === true,
      allowForgiveness: plan?.allowForgiveness === true,
      allowReopeningCloseness: plan?.allowReopeningCloseness === true,
      socialMove: plan?.socialMove ?? null,
      maxWords: Number(plan?.maxWords ?? 0),
      maxSentences: Number(plan?.maxSentences ?? 0),
...
function numberField(
  turn: NaturalCharacterizationTurnArtifact,
  side: "before" | "after",
  key: string,
): number {
...
function genericInvariantEvidence(
  execution: NaturalCharacterizationExecutionResult,
): string[] {
  return execution.turns.flatMap((turn) =>
    turn.invariantViolations.map(
      (code) => `turn=${turn.turnNumber} invariant=${code}`,
    ),
  );
}
...
function evaluateSingleExecution(
  ...
  const execution: NaturalCharacterizationExecutionResult = {
    ...
    classification: scenario.mode === "exploration" ? "OBSERVATION" : "OBSERVATION",
    evidence: [],
  ...
  if (oracle.type === "stop_resume") {
    ...
    for (const turnNumber of oracle.stopTurns) {
      ...
      if (!turn || turn.responsePlan.continueConversation) {
        failures.push(`turn=${turnNumber} expected continueConversation=false`);
      }
    }
    const resume = execution.turns[oracle.resumeTurn - 1];
    if (!resume || !resume.responsePlan.continueConversation) {
      failures.push(`turn=${oracle.resumeTurn} expected explicit resume to reopen conversation`);
    }
    execution.classification = failures.length ? "FAIL_PRODUCT" : "PASS";
    execution.evidence.push(...(failures.length ? failures : ["stop checkpoints closed and explicit resume reopened conversation"]));
    return execution;
  }
  ...
  if (oracle.type === "repair_requires_source") {
    ...
    for (const turn of execution.turns.filter((item) => item.turnNumber >= oracle.watchFromTurn)) {
      ...
      if (after > before && !turn.semantic.repairAttempt && !turn.semantic.apology) {
        failures.push(`turn=${turn.turnNumber} repairProgress ${before}->${after} without repair/apology evidence`);
      }
    }
    execution.classification = failures.length ? "FAIL_PRODUCT" : "PASS";
  ...
  if (oracle.type === "third_party_neutrality") {
    ...
    for (const turnNumber of oracle.thirdPartyTurns) {
      const turn = execution.turns[turnNumber - 1];
      if (!turn) continue;
      if (turn.semantic.target !== "third_party") {
        failures.push(`turn=${turnNumber} expected target=third_party received=${turn.semantic.target}`);
        continue;
      }
      const harmed =
        numberField(turn, "after", "hurtScore") > numberField(turn, "before", "hurtScore") ||
        numberField(turn, "after", "conflictScore") > numberField(turn, "before", "conflictScore") ||
        numberField(turn, "after", "warmthScore") < numberField(turn, "before", "warmthScore") ||
        numberField(turn, "after", "trustScore") < numberField(turn, "before", "trustScore");
      if (harmed) failures.push(`turn=${turnNumber} third-party evidence changed dyadic harm/quality fields`);
    }
    execution.classification = failures.length ? "FAIL_PRODUCT" : "PASS";
    execution.evidence.push(...(failures.length ? failures : ["third-party checkpoints stayed dyad-neutral under high activation"]));
    return execution;
  }

  // Paired relationship comparison is evaluated after both variants exist.
...
function relationshipDistance(
  a: NaturalCharacterizationTurnArtifact,
  b: NaturalCharacterizationTurnArtifact,
): number {
  const keys = ["warmthScore", "trustScore", "conflictScore", "hurtScore", "repairProgress", "toleranceMultiplier"];
  return keys.reduce((sum, key) => {
    const av = numberField(a, "after", key);
    const bv = numberField(b, "after", key);
    return sum + Math.abs(av - bv);
  }, 0);
}

function applyPairedOracle(
  scenario: NaturalCharacterizationScenarioDefinition,
  executions: NaturalCharacterizationExecutionResult[],
) {
  ...
  const distances = Array.from({ length }, (_, index) => relationshipDistance(left.turns[index], right.turns[index]));
  const differentiated = distances.some((distance) => distance > 0.01);
  const finalLeft = left.turns.at(-1);
  const finalRight = right.turns.at(-1);
  if (!finalLeft || !finalRight) return;

  const leftSeed = scenario.variants?.find((item) => item.variantId === left.variantId);
  const rightSeed = scenario.variants?.find((item) => item.variantId === right.variantId);
  const leftInitialHurt = Number(leftSeed?.initialDynamicState?.relationship?.hurtScore ?? 0);
  const rightInitialHurt = Number(rightSeed?.initialDynamicState?.relationship?.hurtScore ?? 0);
  const leftHurtDelta = numberField(finalLeft, "after", "hurtScore") - leftInitialHurt;
  const rightHurtDelta = numberField(finalRight, "after", "hurtScore") - rightInitialHurt;
  ...
  for (const execution of executions) {
    ...
    execution.evidence = [
      ...
      `leftHurtDelta=${leftHurtDelta.toFixed(3)}`,
      `rightHurtDelta=${rightHurtDelta.toFixed(3)}`,
      failure
        ? "expected established trajectory to differ without producing more hurt than fresh trajectory"
        : "same transcript produced distinct seeded relationship trajectory without established over-harm",
    ];
  }
}

export function naturalCharacterizationV2Scenarios() {
  return scenarios;
}

export async function runKairaNaturalCharacterizationV2(
  runId = "v2run001",
): Promise<NaturalCharacterizationV2Report> {
  const executions: NaturalCharacterizationExecutionResult[] = [];

  for (const scenario of scenarios) {
    const variants = scenario.variants?.length
      ? scenario.variants
      : [{ variantId: "default", initialDynamicState: scenario.initialDynamicState }];
    const scenarioExecutions: NaturalCharacterizationExecutionResult[] = [];

    for (const variant of variants) {
      const harnessScenario = scenarioForHarness(scenario, variant.variantId);
    ...
    if (scenario.scenarioId === "S10") {
      for (const execution of scenarioExecutions) {
        const observedNorm = execution.turns.some(
          (turn) => turn.relationship.after?.dyadicNormObserved === "yes",
        );
  ...
  const classCounts: Record<NaturalCharacterizationClass, number> = {
    PASS: 0,
    FAIL_PRODUCT: 0,
  ...
  return {
    ...
    turnCount: executions.reduce((sum, execution) => sum + execution.turns.length, 0),
    classCounts,
    executions,
  };
}
