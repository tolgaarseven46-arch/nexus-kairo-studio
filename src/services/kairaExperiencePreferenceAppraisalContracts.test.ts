...
describe("Kaira experience preference appraisal contracts", () => {
  ...
  it.each([
    ...
    expect(preferenceEvidenceFromExperienceAppraisal(appraisal(overrides))).toEqual({
      status: "rejected",
      evidence: null,
      reason,
    });
  ...
  it("binds produced evidence to the exact lived world-observation provenance", () => {
    const projection = projectExperiencePreferenceEvidenceToLivedMemory(
      livedMemory("obs_1"),
      appraisal({ sourceWorldObservationId: "obs_1" }),
    );
    expect(projection.status).toBe("projected");
    if (projection.status !== "projected") throw new Error("expected projection");
    expect(projection.memory.selfRevisionEvidence).toMatchObject({
      factKey: "preferred_music",
      domain: "preference",
      value: "ambient",
    });
