...
describe("natural-conversation characterization acceptance", () => {
  ...
  it("bounds history amplification and never lets autobiography alter relational meaning", () => {
    ...
    expect(
      withDeepHistory.contextFactors.affectiveNegative /
        withoutHistory.contextFactors.affectiveNegative,
    ).toBeLessThanOrEqual(1.12);
    expect(
      withDeepHistory.contextFactors.activation /
        withoutHistory.contextFactors.activation,
    ).toBeLessThanOrEqual(1.12);
  });
