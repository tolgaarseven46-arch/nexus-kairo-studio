...
function evidenceRows(state: KairaCanonicalIdentityState, factKey: string) {
  return state.autobiographicalMemories
    .filter((memory): memory is KairaAutobiographicalMemory => memory.origin === "lived")
    .map((memory) => ({ memory, evidence: normalizeKairaSelfRevisionEvidence(memory.selfRevisionEvidence) }))
    .filter(({ evidence }) => evidence?.factKey === factKey)
    .map(({ memory, evidence }) => ({ memory, evidence: evidence! }));
}
...
export function evaluateKairaSelfFactRevision(
  ...
  for (const row of rows) {
    const groupKey = valueKey(row.evidence.value);
  ...
  const changingSeed = Boolean(current && current.source === "identity_seed" && valueKey(current.value) !== valueKey(winner[0]?.evidence.value));
  ...
  const evidence = winner[0].evidence;
  const confidence = Math.max(0, Math.min(1, averageConfidence * supportRatio));
  if (current && valueKey(current.value) === valueKey(evidence.value)) {
    return {
      status: "reinforced",
      factKey: key,
