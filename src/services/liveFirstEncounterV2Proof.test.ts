...
describe("LIVE proof: first-encounter continuity + latency v2", () => {
  it("routes contextual iyilik through canonical local well-being reply with bounded live latency", async () => {
    const probeId = `live-proof-${Date.now()}`;
    ...
    const response = await fetch("https://nexus-kairo-studio.onrender.com/api/chat", {
      ...
      body: JSON.stringify({
        ...
        history: [
          { sender: "droit", text: "gayet iyiyim, sen nasılsın?" },
        ],
        provider: "openrouter",
        conversationPhase: "first_encounter",
        firstEncounterContext: {
          roomId: "live-proof-room",
          roomName: "live-proof",
          isOwner: true,
        },
    ...
    console.log("LIVE_FIRST_ENCOUNTER_V2_PROOF", JSON.stringify({
