...
describe("relationship-conditioned appraisal contracts", () => {
  it("maps the same negative event to different tendencies from relationship context", () => {
    const fresh = appraiseRelationshipConditionedEvent({ event, relationship: relationship(), internalState, modulation });
    const close = appraiseRelationshipConditionedEvent({ event, relationship: relationship({ closeness: 82, familiarityDays: 30, interactionCount: 60, warmth: 78, trust: 78, relationshipQuality: 82, conflict: 5, hurt: 8 }), internalState, modulation });
    const damaged = appraiseRelationshipConditionedEvent({ event, relationship: relationship({ closeness: 55, familiarityDays: 30, interactionCount: 60, warmth: 38, trust: 38, relationshipQuality: 35, conflict: 38, hurt: 45, priorConversationState: "distancing", conversationState: "distancing" }), internalState, modulation });
  ...
  it("uses current internal state as appraisal evidence without naming a person", () => {
    const calm = appraiseRelationshipConditionedEvent({ event, relationship: relationship({ conflict: 18, hurt: 22 }), internalState, modulation });
    const activated = appraiseRelationshipConditionedEvent({ event, relationship: relationship({ conflict: 18, hurt: 22 }), internalState: { anger: 95, stress: 90, calmness: 10 }, modulation });
  ...
  it("treats red-line violations as hard-stop independent of healthy closeness", () => {
    const result = appraiseRelationshipConditionedEvent({ event: { ...event, redLine: true }, relationship: relationship({ closeness: 90, familiarityDays: 100, interactionCount: 300, warmth: 90, trust: 90, relationshipQuality: 90 }), internalState, modulation });
    expect(result.reactionTendency).toBe("withdrawn");
  });

  it("keeps repair as a distinct tendency while residual injury exists", () => {
    const result = appraiseRelationshipConditionedEvent({ event: { kind: "neutral", targetsKaira: false, redLine: false, repairSignal: true }, relationship: relationship({ conflict: 25, hurt: 30, repairProgress: 15, conversationState: "repairing" }), internalState, modulation });
