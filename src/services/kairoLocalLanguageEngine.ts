...
function detectIntent(
  ...
  if (
    dialogueMove === "invite_emotional_context" &&
    classifyLocalEmotionalIntent(text)
  ) {
...
export function tryLocalKairoReply(
  ...
  if (!intent) {
    return { handled: false, confidence: 0, source: "ai", normalization };
  }
  ...
  const funny = personality.humor >= 65 && !angry && !hurtMode;
  let pool: string[] = [];

  if (intent === "emotional_opening") {
    pool = hurtMode
      ? ["hayırdır", "ne oldu"]
      : ["hmm niye", "niye ya", "ne oldu", "hayırdır"];
  }
  if (intent === "greeting") {
    pool = hurtMode
      ? ["selam", "hee selam"]
  ...
  if (intent === "what_doing") {
    pool = hurtMode
      ...
      : funny && close
        ? [
            "takılıyorum ya",
            "ne olsun takılıyorum",
            "boştayım sayılır sen napiyon",
            "takılıyorum kanka sen",
          ]
        : funny
          ? [
              "takılıyorum ya",
  ...
  if (intent === "thanks") {
    pool = close
      ? ["eyvallah", "ne demek kanka", "rica ederim ya", "lafı mı olur"]
      : familiar
        ? ["eyvallah", "rica ederim ya", "lafı mı olur"]
  ...
  if (intent === "agreement") {
    pool = hurtMode
      ? ["he", "tamam"]
      : familiar
        ? ["aynen", "heh aynen", "tamamdır", "aynen öyle"]
        : ["tamam", "evet"];
  }
  if (intent === "goodbye") {
    pool = close
      ...
      : familiar
        ? ["hadi görüşürüz", "kendine iyi bak", "görüşürüz"]
        : ["görüşürüz", "hoşça kal"];
  }
  if (intent === "good_night") {
    pool = close
      ? ["iyi geceler kanka", "geceler", "iyi uyu", "hadi iyi geceler"]
      : familiar
        ? ["geceler", "iyi uyu", "hadi iyi geceler"]
  ...
  const reply = chooseLanguageReply(
    userId,
    pool,
    `${intent}|${normalization.canonical}|${state.anger}|${warmth}|${hurt}|${trace.decision.chosenTone}`,
  );
  ...
  return {
    ...
    intent,
    reply,
    confidence: intent === "emotional_opening" ? 0.96 : 0.97,
