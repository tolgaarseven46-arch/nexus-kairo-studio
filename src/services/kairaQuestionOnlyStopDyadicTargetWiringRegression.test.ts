...
describe("question-only stop dyadic target wiring regression", () => {
  it("treats typed question-only stop facets as interlocutor-target evidence without overriding third-party or event scope", () => {
    const source = readFileSync(
      new URL("./socialAppraisalRuntimeProjection.ts", import.meta.url),
      "utf8",
    );
    ...
    expect(source).toContain('relationshipScope === "third_party" || relationshipScope === "event"');
    expect(source).toContain("questionOnlyStopAddressesInterlocutor");
  });
});
