...
describe("Kaira autonomous life worker run coordinator contracts", () => {
  it("runs planning, recovery and due schedule dispatch in one trusted tick", async () => {
    ...
    expect(planning.process).toHaveBeenCalledWith({
    ...
    expect(planning.process.mock.invocationCallOrder[0]).toBeLessThan(recovery.run.mock.invocationCallOrder[0]);
    expect(recovery.run.mock.invocationCallOrder[0]).toBeLessThan(schedules.dispatch.mock.invocationCallOrder[0]);
  ...
  it("continues later stages when planning throws", async () => {
    ...
    expect(recovery.run).toHaveBeenCalledTimes(1);
    expect(schedules.dispatch).toHaveBeenCalledTimes(1);
    expect(result.status).toBe("partial_failure");
    expect(result.planningInbox).toMatchObject({ status: "failed", error: "planning unavailable" });
  });
