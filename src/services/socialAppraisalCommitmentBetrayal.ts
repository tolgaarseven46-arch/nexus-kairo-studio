...
export function applyCommitmentAppraisalEvidence(
  ...
  // Only an explicit canonical current-turn commitment violation activates this
  // concern. An unrelated/unknown attribution or prior world-memory evidence alone
  // must not manufacture betrayal/unfairness or mutate ordinary G4 appraisal.
  if (semantic.attribution?.commitmentViolation !== "present") return { ...base };
