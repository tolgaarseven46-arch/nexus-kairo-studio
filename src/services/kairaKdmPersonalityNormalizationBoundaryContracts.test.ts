...
describe('KDM personality normalization boundary', () => {
  it('keeps direct KDM state math finite for non-finite personality input', () => {
    ...
    expect(Number.isFinite(result.trace.relationship.warmthScore)).toBe(true);
    expect(Number.isFinite(result.trace.relationship.trustScore)).toBe(true);
  });

  it('clamps direct out-of-range personality before KDM relationship math', () => {
    const result = analyzeKdmInteraction('sen salaksın', { patience: -100, empathy: 200, anger: 200, emotionalSensitivity: 200, loyalty: 200 });
