...
describe("text episode + modality regression", () => {
  it("canonical semantics can represent multiple bounded propositions in one episode", () => {
  ...
  it("every proposition carries typed utterance modality so non-facts remain distinguishable", () => {
  ...
  it("proposition contract preserves identity/time/confidence/provenance instead of flattening the episode", () => {
  ...
  it("canonical normalizer preserves bounded proposition modality at runtime", () => {
    const raw = {
      ...baseInterpretation,
      propositions: [
    ...
    expect(normalized.propositions?.map((item) => item.modality)).toEqual(["assertion", "assertion", "question", "wish", "prediction"]);
  });

  it("canonical normalizer enforces a hard proposition bound", () => {
    const raw = {
      ...baseInterpretation,
  ...
  it("semantic provider contract asks for proposition-level output instead of only one flattened intent", () => {
