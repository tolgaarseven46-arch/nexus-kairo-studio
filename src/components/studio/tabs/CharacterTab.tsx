...
const EMOTIONAL_TRAITS: TraitConfig[] = [
  { key: 'anger', label: 'Sinirlilik', low: 'Sakin', high: 'Çabuk Tepki', icon: ShieldCheck },
  { key: 'emotionalSensitivity', label: 'Hassasiyet', low: 'Mesafeli', high: 'Hassas', icon: Heart },
  { key: 'socialIntelligence', label: 'Sosyal Zekâ', low: 'Direkt', high: 'Sezgisel', icon: Users },
  { key: 'charisma', label: 'Karizma', low: 'Sade', high: 'Etkileyici', icon: Sparkles },
];
        ...
        <header className="shrink-0 h-14 rounded-xl border border-zinc-800 bg-zinc-900/70 px-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-zinc-500 truncate">Kaira'nın sabit karakterini ve kişilik eğilimlerini düzenle</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[260px_minmax(420px,1fr)_280px] gap-3 overflow-hidden">
          ...
          <section className="min-h-0 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/60 flex flex-col">
            <div className="p-4 border-b border-zinc-800/80">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">Kimlik</p>
                  <h2 className="text-sm font-semibold mt-1">Kaira</h2>
                </div>
                <button type="button" onClick={() => setEditingIdentity((v) => !v)} className="p-1.5 rounded-md text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800" title="Kimliği düzenle">
                  <Edit3 className="w-3.5 h-3.5" />
                </button>
              </div>
              <div className="flex justify-center mb-3">
                <div className="w-24 h-24 rounded-2xl border border-indigo-500/20 bg-zinc-950 overflow-hidden shadow-inner">
                  <DroitAvatar expression={expression} size="lg" />
                </div>
              </div>
              {editingIdentity ? (
                <div className="space-y-2">
                  {(['name', 'role', 'origin', 'bio'] as const).map((key) => (
                    <input
                      key={key}
                      value={identity[key]}
                      onChange={(e) => setIdentity((prev) => ({ ...prev, [key]: e.target.value }))}
                      className="w-full rounded-md border border-zinc-700 bg-zinc-950 px-2.5 py-2 text-[10px] text-zinc-200 outline-none focus:border-indigo-500"
                      placeholder={key}
                    />
                  ))}
            ...
            <div className="p-4 min-h-0 flex-1 overflow-hidden">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">Karakter kuralları</p>
                  <p className="text-[9px] text-zinc-600 mt-0.5">Değişmemesi gereken temel prensipler</p>
                </div>
                <button type="button" onClick={() => setEditingRules((v) => !v)} className="p-1.5 rounded-md text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800" title="Kuralları düzenle">
                  <Edit3 className="w-3.5 h-3.5" />
                </button>
              </div>
              {editingRules ? (
                <div className="h-[calc(100%-42px)] flex flex-col gap-2">
                  <textarea
                    value={rulesDraft}
                    onChange={(e) => setRulesDraft(e.target.value)}
                    className="flex-1 min-h-0 w-full resize-none rounded-lg border border-zinc-700 bg-zinc-950 p-2.5 text-[10px] leading-relaxed text-zinc-300 outline-none focus:border-indigo-500"
                  />
                  <button type="button" onClick={saveRules} className="h-7 rounded-md bg-zinc-800 hover:bg-zinc-700 text-[10px]">Kuralları Kaydet</button>
                </div>
              ) : (
                <div className="space-y-1.5 overflow-hidden">
                  {rules.slice(0, 5).map((rule, index) => (
                    <div key={`${rule}-${index}`} className="flex gap-2 rounded-lg border border-zinc-800/70 bg-zinc-950/40 p-2">
                      <span className="text-[9px] font-mono text-indigo-400/70 mt-0.5">0{index + 1}</span>
                      <p className="text-[9.5px] leading-relaxed text-zinc-400">{rule}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <section className="min-h-0 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/60 flex flex-col">
            <div className="px-4 py-3 border-b border-zinc-800/80 flex items-center justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">Temel kişilik</p>
                <h2 className="text-sm font-semibold mt-1">Kaira'nın davranış eğilimleri</h2>
              </div>
              <span className="text-[9px] text-zinc-600">0 — 100</span>
            </div>
            ...
            <div className="flex-1 min-h-0 p-3 overflow-hidden grid grid-cols-1 sm:grid-cols-2 gap-2.5 auto-rows-min content-start">
              {CORE_TRAITS.map((trait) => {
                ...
                return (
                  <div key={trait.key} className="rounded-lg border border-zinc-800/90 bg-zinc-950/35 px-3 py-2.5">
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <Icon className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                        <span className="text-[11px] font-medium text-zinc-200 truncate">{trait.label}</span>
                      </div>
                      <span className="text-[10px] font-mono text-indigo-300">{value}</span>
                    </div>
                    <input
                      ...
                      max="100"
                      value={value}
                      onChange={(e) => onPersonalityChange({ [trait.key]: Number(e.target.value) } as Partial<DroitPersonalityTraits>)}
            ...
            <div className="shrink-0 border-t border-zinc-800/80 px-4 py-2.5">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Karakter özeti</p>
                  <p className="text-[10px] text-zinc-400 truncate">{summary.dominant.map((item) => item.label).join(' • ')}</p>
                </div>
                <span className="text-[9px] px-2 py-1 rounded-md bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 shrink-0">{summary.dominant[0]?.label || 'Dengeli'} baskın</span>
              </div>
            </div>
          </section>

          {/* Speech + emotional character */}
          <section className="min-h-0 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/60 flex flex-col">
            <div className="p-4 border-b border-zinc-800/80">
              <p className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">Konuşma & üslup</p>
              <h2 className="text-sm font-semibold mt-1">Kişiliğin dışa vurumu</h2>
              <div className="mt-3 space-y-2.5">
                {[
                  ['Uzunluk', personality.communication, 'Kısa', 'Detaylı'],
                  ['Samimiyet', personality.empathy, 'Mesafeli', 'Samimi'],
                  ['Enerji', personality.initiative, 'Sakin', 'Enerjik'],
                  ['Mizah', personality.humor, 'Ciddi', 'Esprili'],
                ].map(([label, rawValue, low, high]) => {
                  const value = clamp(rawValue as number);
                  return (
                    <div key={String(label)}>
                      <div className="flex justify-between text-[9px] mb-1"><span className="text-zinc-400">{label}</span><span className="font-mono text-zinc-500">{value}</span></div>
                      <div className="h-1.5 rounded-full bg-zinc-800 overflow-hidden"><div className="h-full rounded-full bg-indigo-500" style={{ width: `${value}%` }} /></div>
                      <div className="flex justify-between text-[7.5px] text-zinc-600 mt-0.5"><span>{low}</span><span>{high}</span></div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="p-4 min-h-0 flex-1 overflow-hidden">
              <p className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">Duygusal karakter</p>
              <div className="mt-2 space-y-2">
                {EMOTIONAL_TRAITS.map((trait) => {
                  const value = clamp(personality[trait.key]);
                  const Icon = trait.icon;
                  return (
                    <div key={trait.key} className="rounded-lg border border-zinc-800/80 bg-zinc-950/30 p-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5"><Icon className="w-3 h-3 text-indigo-400" /><span className="text-[9.5px] text-zinc-300">{trait.label}</span></div>
                        <span className="text-[9px] font-mono text-zinc-500">{value}</span>
                      </div>
                      <div className="mt-1.5 h-1 rounded-full bg-zinc-800 overflow-hidden"><div className="h-full rounded-full bg-zinc-500" style={{ width: `${value}%` }} /></div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-3 rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
                <p className="text-[9px] uppercase tracking-wider text-zinc-600">Otomatik profil</p>
                <div className="mt-2 grid grid-cols-1 gap-1.5 text-[9px]">
                  <div className="flex justify-between"><span className="text-zinc-500">Ton</span><span className="text-zinc-300">{summary.tone}</span></div>
                  <div className="flex justify-between"><span className="text-zinc-500">Cevap yapısı</span><span className="text-zinc-300">{summary.speech}</span></div>
                  <div className="flex justify-between"><span className="text-zinc-500">Sosyal yaklaşım</span><span className="text-zinc-300">{summary.social}</span></div>
                  <div className="flex justify-between"><span className="text-zinc-500">Sabır</span><span className="text-zinc-300">{traitWord(clamp(personality.patience))}</span></div>
                </div>
              </div>
            </div>
          </section>
